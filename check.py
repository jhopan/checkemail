...
PENTING: client = CamofoxClient HTTP dari check_google_accounts.py
(server camofox-browser di localhost:9377) - BUKAN Camoufox lokal.
Versi Camoufox-lokal bermasalah (OOM 3GB + hang) - jangan dipakai.
"""

import os
import sys

# Import dari SATU sumber kebenaran: check_google_accounts.py (HTTP client)
from check_google_accounts import (
    CamofoxClient,
    check_account,
    do_logout,
    generate_report,
...
def cli():
    ...
    # Pilih loader sesuai mode (loader dari check_google_accounts.py)
    if args.mode == 'gmail' or (args.mode == 'auto' and args.csv):
        csv_file = args.csv or 'gmail_accounts.csv'
        accounts = read_accounts_direct(csv_file, args.password)
    elif args.mode == 'unud':
        csv_file = args.csv or 'students.csv'
        accounts = read_students(csv_file)
    else:
        csv_file = args.csv or 'gmail_accounts.csv'
        # auto: coba direct dulu, kalau tidak ada @ -> unud
        accounts = read_accounts_direct(csv_file, args.password)
        if not accounts:
            accounts = read_students(csv_file)
